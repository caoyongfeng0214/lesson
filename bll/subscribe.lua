...
subscribe.state = function( where, group, order, cn )
    local sql = [[SELECT ( SELECT COUNT(1) FROM package2lesson pl WHERE p.`id` = pl.`packageId` ) AS lessonCount, 
        ...
        CONCAT("[", (SELECT SUBSTRING_INDEX(GROUP_CONCAT(JSON_OBJECT("lessonUrl", pl.`lessonUrl`, "learnedFlag", (SELECT COUNT(1) FROM testrecord t WHERE username = 'keep' AND lessonUrl = pl.`lessonUrl` AND emptyCount = 0) ) ),",{",3) FROM package p LEFT JOIN package2lesson pl ON p.`id` = pl.`packageId` LEFT JOIN subscribe s ON p.`id` = s.`packageId` WHERE s.username = sb.username AND s.packageId = sb.packageId ), "]" ) lessons,
